...
const Nav = () => {
  ...
  return (
    ...
    bottom-0 mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:w-md xl:h-screen
     '>
      {/* inner */}
      <div 
      className='flex w-full xl:flex-col items-center justify-between 
      xl:justify-center gap-y-10 px-4  md:px-40 xl:px-0 h-[30px]  xl:h-max py-8 bg-white/10
  ...
  {navData.map((link, index) => {
    ...
    duration-300`} 
      ...
      >
        ...
        {/* icon */}
      {/* <div>{link.icon}</div> */}
      <div className="text-xl xl:text-lg">{link.icon}</div>
      </Link>
    )
  })}
</div>
